...
```{r}
sets <- list()
sets[[const$train]] <- ultimatraining[,2:dim(ultimatraining)[2]]
sets[[const$test]] <- ultimotest[,2:dim(ultimotest)[2]]
sets[[const$train]]$C <- as.factor(sets[[const$train]]$C)
m <- entrenar_arbol_c45(sets, "C")
KaggleWiteData(1:dim(ultimotest[,2:dim(ultimotest)[2]])[1], m$prediction, path = "predictions/")
```
...
library(RWeka)
library(caret)
hr_base_model <- rpart(C ~ ., data = ultimatraining[,2:dim(ultimatraining)[2]], method = "class",
...
test$pred <- predict(hr_base_model, ultimotest[,2:dim(ultimotest)[2]], type = "class")
...
KaggleWiteData(1:dim(ultimotest[,2:dim(ultimotest)[2]])[1], test$pred, path = "predictions/")
```

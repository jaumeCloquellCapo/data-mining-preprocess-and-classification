...
#### Best solution for now (Amelia 5 + IPF + SMOTE")
#$treeC45
#$treeC45$accuracy
#[1] 0.9681316
```
...
```{r}
sets <- list()
sets[[const$train]] <- cleantrain[,2:dim(cleantrain)[2]]
sets[[const$test]] <- cleantest[,2:dim(cleantest)[2]]
sets[[const$train]]$C <- as.factor(sets[[const$train]]$C)
m <- entrenar_arbol_c45(sets, "C")
KaggleWiteData(1:dim(cleantest[,2:dim(cleantest)[2]])[1], m$prediction, path = "predictions/")
```
...
library(RWeka)
library(caret)
hr_base_model <- rpart(C ~ ., data = cleantrain[,2:dim(cleantrain)[2]], method = "class",
...
test$pred <- predict(hr_base_model, cleantest[,2:dim(cleantest)[2]], type = "class")
